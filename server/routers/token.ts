...
export const tokenRouter = router({
  ...
  test: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        return { ok: true as const, userId: info.id, userName: info.name, scopes: info.scopes ?? [], expiresAt: info.expiresAt ?? null };
      } catch (err: any) {
  ...
  save: protectedProcedure
    .input(z.object({ accessToken: z.string().min(10), adAccountId: z.string().min(1), tokenLabel: z.string().optional() }))
    .mutation(async ({ ctx, input }) => {
      const info = await validateToken(input.accessToken);
      await saveToken({ userId: ctx.user.id, accessToken: input.accessToken, adAccountId: input.adAccountId, tokenLabel: input.tokenLabel ?? null, scopes: info.scopes?.join(",") ?? null, expiresAt: info.expiresAt ? new Date(info.expiresAt) : null });
      await addTokenHistory({ userId: ctx.user.id, action: "save", tokenLabel: input.tokenLabel ?? null, detail: `Connected ad account ${input.adAccountId}` });
  ...
  activate: protectedProcedure
    .input(z.object({ id: z.number() }))
    .mutation(async ({ ctx, input }) => {
      await activateToken(ctx.user.id, input.id);
      await addTokenHistory({ userId: ctx.user.id, action: "activate", detail: `Switched to token #${input.id}` });
      return { ok: true };
    }),
});
